...
- Recurso (x-ui.resource): MUST conter `idField`, `idFieldValid`, `readOnly`, `capabilities`; `idFieldMessage` e opcional e presente quando `idFieldValid=false`. Em schemas de resposta, `identity` e opt-in e, quando presente, MUST conter `valid` e ao menos `keyField`, `titleField` ou `displayLabelField`; schemas de request MUST NOT publica-lo.
- Chart (x-ui.chart):
...
- Fixtures de validacao:
  ...
  - `examples/x-ui-field-field-access-reason-only.invalid.json`
