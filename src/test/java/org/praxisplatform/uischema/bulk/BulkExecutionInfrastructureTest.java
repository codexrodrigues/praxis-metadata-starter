...
class BulkExecutionInfrastructureTest {
    ...
    void explicitNamespaceAndBindingAreRequiredWithoutDatabaseAccess() {
        ...
        var binding = new BulkExecutionInfrastructure(ds, manager, "tenant-a:prod");
        ...
        for (String invalid : new String[] {null, "", " ", " leading", "trailing ", "tab\tvalue", "x".repeat(201)}) {
            assertThatThrownBy(() -> new BulkExecutionInfrastructure(ds, manager, invalid)).isInstanceOf(IllegalArgumentException.class);
        }
    ...
    void rejectsUnsupportedOrMismatchedManagersAndUnprovenWrappers() {
        ...
        assertThatThrownBy(() -> new BulkExecutionInfrastructure(ds, mock(PlatformTransactionManager.class), "x"))
                .isInstanceOf(IllegalArgumentException.class);
        assertThatThrownBy(() -> new BulkExecutionInfrastructure(ds, new DataSourceTransactionManager(mock(DataSource.class)), "x"))
                .isInstanceOf(IllegalArgumentException.class);
        for (var wrapper : new DataSource[] {new DelegatingDataSource(ds), mock(AbstractRoutingDataSource.class)}) {
            assertThatThrownBy(() -> new BulkExecutionInfrastructure(wrapper, new DataSourceTransactionManager(wrapper), "x"))
                    .isInstanceOf(IllegalArgumentException.class);
        }
        verifyNoInteractions(ds);
    }
    ...
    void jpaManagerCannotClaimADifferentDatasourceThanItsEntityManagerFactory() {
        ...
        assertThatThrownBy(() -> new BulkExecutionInfrastructure(ds, manager, "x")).isInstanceOf(IllegalArgumentException.class);
        when(((EntityManagerFactoryInfo) emf).getDataSource()).thenReturn(ds);
        assertThat(new BulkExecutionInfrastructure(ds, manager, "x").dataSource()).isSameAs(ds);
        ...
        assertThatThrownBy(() -> new BulkExecutionInfrastructure(ds, opaque, "x")).isInstanceOf(IllegalArgumentException.class);
        verifyNoInteractions(ds);
    }
